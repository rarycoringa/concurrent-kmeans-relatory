...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{img/charts/chart4_jfr_hot_methods}
    \caption{Distribuição do tempo de CPU na execução serial (JFR, 11.054 amostras):
             o parsing CSV concentra $\approx$82\% do tempo total, deixando apenas
             $\approx$8\% para o algoritmo K-Means.}
    \label{fig:chart-jfr-hot-methods}
\end{figure}
...
\textbf{GC.}
Foram realizadas 1.099 coletas de GC em 118 s, com pausa total de 805 ms ---
apenas 0,68\% do tempo de execução. O heap oscila entre $\approx$150 MB e
$\approx$5 MB após cada coleta, reflexo da alta taxa de criação e descarte de
objetos \texttt{String} durante o parsing. O GC não é um gargalo.
...
Uma implementação serial em Erlang não foi realizada. A linguagem é projetada
em torno do modelo de atores: processos são a unidade fundamental de execução
e toda computação significativa ocorre por troca de mensagens entre eles.
Escrever um K-Means verdadeiramente serial em Erlang exigiria contornar
deliberadamente os mecanismos centrais da linguagem, produzindo código
artificial e não representativo. A implementação Erlang é apresentada
diretamente na Seção~\ref{sec:concurrent} como versão concorrente.
...
O ParallelGC tem o maior throughput em todos os níveis de carga, seguido pelo G1GC.
O ZGC fica levemente atrás em todos os casos pelas barreiras de escrita do seu
algoritmo. No geral, o impacto do GC é pequeno --- as pausas somam menos de 1\%
do tempo total.
