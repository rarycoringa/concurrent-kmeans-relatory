...
elimina a necessidade de sincronização sem uso de \texttt{synchronized} ou variáveis
atômicas. A coordenadora aguarda todos os resultados antes de prosseguir, garantindo
que não há leitura de dados ainda sendo escritos.

Uma cópia dos centróides é tirada antes do início de cada iteração, garantindo que
todos os workers trabalhem com o mesmo estado durante aquela iteração.
...
\begin{itemize}
    ...
    \item \textbf{v2-hybrid} --- pool fixo de $N$ platform threads como workers
          e uma virtual thread como coordenadora. A virtual thread suspende
          automaticamente durante operações de I/O e durante a espera pelos
          resultados dos workers, liberando o processador para outras tarefas.
\end{itemize}
...
JCS1 e JCS2 não produziram nenhum resultado inválido: a leitura concorrente é segura.
JCS3 e JCS4 falharam como esperado: atualizações perdidas ocorrem em até 28\% das
amostras quando os arrays são compartilhados sem proteção.
...
porque suas virtual threads são multiplexadas sobre um número menor de threads reais,
evitando a sobrecarga do sistema operacional com muitas threads concorrentes. Em 16
threads todas convergem para $\approx$4,5 exec/min. O ZGC fica levemente abaixo dos
demais em todos os casos pelo custo adicional do seu algoritmo de coleta.
...
O coordenador virtual suspende durante leituras de arquivo e enquanto aguarda os
workers, tornando o parsing CSV invisível ao JFR. Os 16 workers de plataforma
concentram 99,9\% das amostras, com \texttt{squaredDistance} e
\texttt{closestCentroid} no topo. GC: 1.072 coletas em 140 s (0,84\% de pausa).
Não é gargalo.
...
v2-virtual supera as demais (4,00 exec/min): ao multiplexar workers sobre um
número menor de threads reais, evita a sobrecarga de escalonamento que aparece
com muitas threads de plataforma simultâneas. Em 16 threads, todas convergem
para $\approx$4,5 exec/min.
