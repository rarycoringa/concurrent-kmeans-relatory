...
medição de 10 s e 1 fork por benchmark. O estado do dataset é inicializado uma vez
por trial (\texttt{@Setup(Level.Trial)}) e os vetores de hotspot usam semente fixa
(\texttt{seed = 42}) para reprodutibilidade.
...
necessidade de ferramentas externas. São capturados: tempo de CPU acumulado
(\texttt{erlang:statistics(runtime)}), tempo de parede
(\texttt{erlang:statistics(wall\_clock)}), número de coletas GC e palavras
reclamadas (\texttt{erlang:statistics(garbage\_collection)}), e utilização média
dos schedulers BEAM (\texttt{erlang:statistics(scheduler\_wall\_time)}). O profile
cobre uma execução completa do algoritmo com $k=10$ e \texttt{maxIter}=10.