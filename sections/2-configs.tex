...
(2 $\times$ 8 GB), com velocidade máxima de 5600 MT/s e frequência configurada
em 4800 MT/s.
...
\paragraph{\sout{Erlang:}}
\sout{\textbf{Erlang/OTP} com emulador BEAM versão \texttt{13.2.2.5}.}
\fi % ============================================================
...
Os microbenchmarks foram implementados com \textbf{JMH 1.37} e executados sobre
\textbf{JDK 25.0.1}. As anotações de classe definem a política global aplicada
a todos os benchmarks:

\begin{itemize}
    \item \texttt{@BenchmarkMode(Mode.AverageTime)} — mede o tempo médio por
          operação;
    \item \texttt{@OutputTimeUnit(TimeUnit.SECONDS)} — resultados expressos em
          segundos;
    \item \texttt{@Warmup(iterations = 1, time = 10)} — 1 iteração de aquecimento
          de 10 s para estabilizar a compilação JIT antes das medições;
    \item \texttt{@Measurement(iterations = 3, time = 10)} — 3 iterações de
          medição de 10 s cada;
    \item \texttt{@Fork(1)} — 1 processo JVM isolado por benchmark, eliminando
          interferência de estado entre casos.
\end{itemize}

Foram definidos quatro casos de benchmark, organizados em dois grupos de estado
(\texttt{@State}):

\begin{itemize}
    \item \textbf{DatasetState} — carrega o caminho e o cabeçalho do arquivo CSV
          uma vez por trial via \texttt{@Setup(Level.Trial)}; o caminho é
          parametrizável via \texttt{@Param};
    \item \textbf{HotspotState} — gera vetores aleatórios de 38 dimensões e
          \(k = 10\) centróides com semente fixa (\texttt{seed = 42}) para
          reprodutibilidade.
\end{itemize}
...
executados sobre \textbf{JDK 25.0.1}. O JCStress instancia múltiplos processos JVM
(\textit{forks}) e varia a combinação de compiladores (Interpreter, C1, C2) e a
afinidade de CPUs entre forks, maximizando a cobertura de interleavings possíveis.
...
Os eventos extraídos com \texttt{jfr print --events} cobrem quatro áreas:

\begin{itemize}
    \item \texttt{jdk.ExecutionSample} — amostras de CPU por método (hot methods);
    \item \texttt{jdk.GarbageCollection}, \texttt{jdk.GCHeapSummary} — pausas e uso
          de heap ao longo da execução;
    \item \texttt{jdk.JavaThreadStatistics}, \texttt{jdk.ThreadCPULoad} — carga de CPU
          por thread e número de threads ativas;
    \item \texttt{jdk.FileRead} — eventos de leitura de arquivo (bytes lidos, duração).
\end{itemize}